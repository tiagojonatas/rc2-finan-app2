...
router.get('/', requireAuth, async (req, res) => {
  ...
  try {
    ...
    res.render('fixed-expenses', { expenses, error: null, success: null });
  } catch (error) {
    console.error(error);
    res.render('fixed-expenses', { expenses: [], error: 'Erro ao carregar despesas fixas', success: null });
  }
});

router.get('/add', requireAuth, (req, res) => {
  res.render('add-fixed-expense', { error: null });
});
...
router.post('/add', requireAuth, async (req, res) => {
  ...
  if (!dueDay || dueDay < 1 || dueDay > 31) {
    return res.render('add-fixed-expense', { error: 'Dia de vencimento deve estar entre 1 e 31' });
  }
  ...
  try {
    ...
    console.error(error);
    res.render('add-fixed-expense', { error: 'Erro ao cadastrar despesa fixa' });
  }
});
...
router.get('/edit/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    return res.render('edit-fixed-expense', { expense: expenses[0], error: null });
  } catch (error) {
    console.error(error);
    return res.redirect('/fixed-expenses');
  }
...
router.post('/edit/:id', requireAuth, async (req, res) => {
  ...
  if (!dueDay || dueDay < 1 || dueDay > 31) {
    const [expenses] = await db.query('SELECT * FROM fixed_expenses WHERE id = ? AND user_id = ?', [expenseId, userId]);
    return res.render('edit-fixed-expense', { expense: expenses[0], error: 'Dia de vencimento deve estar entre 1 e 31' });
  }
  ...
  try {
    ...
    console.error(error);
    const [expenses] = await db.query('SELECT * FROM fixed_expenses WHERE id = ? AND user_id = ?', [expenseId, userId]);
    res.render('edit-fixed-expense', { expense: expenses[0], error: 'Erro ao editar despesa fixa' });
  }
});
